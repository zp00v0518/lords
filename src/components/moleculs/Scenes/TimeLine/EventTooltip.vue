<template>
  <div class="event__tooltip" :style="{top: position+'px'}">
    <div class="event__tooltip__item">{{upperFirstSymbol(gloss.timeLine.upgrade.txt)}} : {{gloss.mine.type[this.type].name.txt}}</div>
    <div class="event__tooltip__item">{{upperFirstSymbol(gloss.timeLine.timeEnd.txt)}}:{{formDataToView(dataEvent.end)}}</div>
  </div>
</template>
