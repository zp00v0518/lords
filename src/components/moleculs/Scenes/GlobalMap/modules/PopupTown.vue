<template>
  <div class="popup-town" :style="getStyle" @click.stop>
    <template v-if="isSelf">
      <button class="popup-town__item" @click.stop="handlerClick('heroTransferDialog')">
        {{ upperFirstSymbol(gloss.popup.sendHero.btn.txt) }}
      </button>
      <button
      ...
      >
        {{ upperFirstSymbol(gloss.popup.sendСaravan.btn.txt) }}
      </button>
      <TownBuilding
        v-if="showMarket"
    ...
    <template v-else>
      <div>
        <button class="popup-town__item" @click.stop="handlerClick('attackEnemyRegion')">
          {{ upperFirstSymbol(gloss.popup.attackEnemyRegion.btn.txt) }}
        </button>
      </div>
    </template>
  </div>
...
<script>
import TownBuilding from '../../TownMap/Building.vue'
